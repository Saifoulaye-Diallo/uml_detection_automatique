...
Usage:
    python compare_image.py --student student.png
                            --reference solution.json
    python compare_image.py --student diagram.jpg
                            --reference ref.json --diff output.json
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Compare un diagramme UML (image) à une solution de "
            "référence (JSON)."
        ),
...
Exemples:
  python compare_image.py --student student.png
                          --reference solution.json
  python compare_image.py --student diagram.jpg
                          --reference ref.json --diff my_diff.json
...
Le fichier de sortie contiendra les différences détaillées :
  ...
  - Problèmes de nommage
    ...
    parser.add_argument(
        ...
        default='diff.json',
        help=(
            'Fichier de sortie pour les différences '
            '(défaut: diff.json)'
        )
    )
    ...
    logger.info(
        f"Chargement de la référence depuis '{args.reference}'..."
    )
    with open(args.reference, encoding='utf-8') as f:
        ref_json = f.read()

    logger.info(
        f"Analyse de l'image '{args.student}' avec GPT-4o Vision..."
    )
    logger.info("Cela peut prendre 15-30 secondes...")

    try:
        diff = extract_uml_json_from_image(
            args.student, reference_json=ref_json
        )
    ...
    logger.info("=" * 60)
    logger.info(
        f"Analyse terminée ! Total d'erreurs détectées : "
        f"{total_errors}"
    )
    logger.info("=" * 60)
    ...
    if total_errors == 0:
        ...
        logger.warning(
            f"{total_errors} différence(s) trouvée(s). "
            f"Consultez le fichier diff.json."
        )
